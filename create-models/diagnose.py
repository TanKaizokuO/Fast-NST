...
def train(args):
    ...
    while step < args.max_steps:
        ...
        for batch_idx, content in enumerate(pbar):
            ...
            # Check for NaN
            if torch.isnan(total_loss) or total_loss.item() > 1e12:
                print(f"\n⚠️  WARNING: Unstable loss detected: {total_loss.item():.2e}")
                print("This usually means style_weight is too high!")
                print("Current settings:")
                print(f"  style_weight: {args.style_weight:.2e}")
                print(f"  content_weight: {args.content_weight}")
                print(f"  tv_weight: {args.tv_weight:.2e}")
                print(
                    "\nRecommendation: Stop training and reduce --style-weight to 1e6 or 1e7"
                )
                if step < 100:
                    print("Stopping training due to early instability...")
                    return
